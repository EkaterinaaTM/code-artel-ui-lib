...
const getStyleVariant = (theme, themeKey, component) => {
  try {
    if (has.call(theme, themeKey)) {
      if (
        has.call(theme[themeKey], "rs") &&
        has.call(theme[themeKey]["rs"], component)
      ) {
...
export class SelectBase extends Component {
  ...
  componentWillReceiveProps = nextProps => {
    ...
    if (
      ...
      this.setState({
        selectedOption: this.getSelectedValueFromOptions(
          options,
          value,
          valueKey
        )
  ...
  onChange = selectedOption => {
    ...
    this.setState(
      ...
      () => {
        input.onChange(
          this.state.selectedOption ? this.state.selectedOption[valueKey] : null
        );
      }
    );
  };
